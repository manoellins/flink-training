...
public class LongRidesExercise {
    ...
    public static class AlertFunction extends KeyedProcessFunction<Long, TaxiRide, Long> {
        ...
        public void open(Configuration config)  {
            ...
            rideStatus = getRuntimeContext().getState(rideStateDescriptor);
        }

        @Override
        public void processElement(TaxiRide ride, Context context, Collector<Long> out) throws Exception {
            TaxiRide firstRideEvent = rideStatus.value();

            if (firstRideEvent == null) {
                rideStatus.update(ride);
                ...
                if (ride.isStart) {
                    ...
                    context.timerService().registerEventTimeTimer(checkTimerTime(ride));
                }
            } else {
                if (ride.isStart) {
                    if (rideIsLong(ride, firstRideEvent)) {
                        out.collect(ride.rideId);
                    }
                } else {
                    context.timerService().deleteEventTimeTimer(checkTimerTime(firstRideEvent));

                    if (rideIsLong(firstRideEvent, ride)) {
                        out.collect(ride.rideId);
                    }
                }
        ...
        public void onTimer(long timestamp, OnTimerContext context, Collector<Long> out)
                throws Exception {
            out.collect(rideStatus.value().rideId);
            rideStatus.clear();
        }

        private boolean rideIsLong(TaxiRide startEvent, TaxiRide endEvent) {
